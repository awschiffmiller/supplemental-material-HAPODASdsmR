...
soap_check <- function(bnd, knots=NULL, data=NULL, plot=TRUE,
   ...
   if(plot){
      ...
      if(!islands){
         plot(bnd[[1]], type="l", main="light blue indicates soap film surface", asp=1)
         lapply(bnd, polygon, col="light blue")
      }else{
         outer_bnd <- bnd[[outer_ind]]
         other_bnd <- bnd
         other_bnd[[outer_ind]] <- NULL
         plot(outer_bnd, type="n", main="light blue indicates soap film surface", asp=1)
         # plot the outer loop
         polygon(outer_bnd, col="light blue")
   ...
   if(!is.null(knots)){
      ...
      if(plot){
         points(knots, col="#1b9e77", pch=20, cex=0.25)
   ...
   if(!is.null(data)){
      ...
      if(plot) points(data, col="#7570b3", pch=20, cex=0.25)
   }
